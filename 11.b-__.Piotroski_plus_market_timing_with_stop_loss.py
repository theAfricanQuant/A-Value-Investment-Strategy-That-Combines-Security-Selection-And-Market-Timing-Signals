...
1. Piotroski F-Score is a no. between 0 - 9 which is used to assess strength of company's financial position[1]
  ...
  Profitability:
       Return on Assets normalized by Total Assets at the start of the year. (1 point if it is greater than 0 in the current year, 0 otherwise);
       Cash Flow from Operations normalized by Total Assets at the stard of the year. (1 point if it is positive in the current year, 0 otherwise);
       Growth in Return on Assets (ROA) over prevous year (1 point if it is greater than 0 in the current year, 0 otherwise);
       Cash Flow from Operation (less) ROA (1 point if it is greater than 0 in the current year, 0 otherwise);
   
   Leverage, Liquidity and Source of Funds
       Growth in Leverage (long-term) ratio (1 point if it is greater than 0 in the current year, 0 otherwise);
       Growth in Current ratio (1 point if it is greater than 0 in the current year, 0 otherwise);
       No new equity issue during the year (1 point if True, 0 otherwise);
   
   Operating Efficiency
       Growth in Gross Margin (1 point if it is greater than 0 in the current year, 0 otherwise);
       Growth in Asset Turnover ratio (1 point if it is greater than 0 in the current year, 0 otherwise);

2. Relative Strength Index (RSI):
   Classified as a momentum Oscillator, RSI charts the current and historical strength or weakness of a stock or market based on the closing prices of a recent trading period
...
3. Money Flow Index (MFI):
   Again an Oscilator, is used to show the money flow (an approximation of the dollar value of a day's trading) over several days.
...
4. Average Directional Movement Index (ADX):
   Indicates strength in current series of prices of a financial instrument. It is a combination of two other positive directional indicator positive/ negative Directional Indicator (or +/ -DI).
   UpMove = h_t - h_(t-1)
   DownMove = l_{t-1} - l_t
...
ref:
    ...
    3. https://en.wikipedia.org/wiki/Money_flow_index
    4. https://en.wikipedia.org/wiki/Average_directional_movement_index
"""
